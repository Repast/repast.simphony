...
public class DisplayOGL2D extends AbstractDisplay implements CanvasListener, PickListener, ChangeListener {
	...
	public DisplayOGL2D(DisplayData<?> data, Layout<?, ?> layout) {
		...
		try {
			...
			MessageCenter.getMessageCenter(getClass()).error("Error initializing OGL 2D display. "
	 ...
	 * (non-Javadoc)
	 * 
	 * @see repast.simphony.visualization.IDisplay#createEditor(javax.swing.JPanel)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see repast.simphony.visualization.IDisplay#setLayout(repast.simphony.
	 ...
	 * interface.
	 * 
	 * @param obj the object to find the layer for
	 * 
	 ...
	 * 
	 * @param clazz the class to register the style for
	 ...
	 * 
	 * @param style the styling info for displaying the layer
	 ...
	 *         or null the VSpatial cannot be found.
	 */
		...
		for (VSpatial spatial : evt.getPicked()) {
			...
			if (obj != null) {
				...
				if (spatial.getProperty(ValueLayerDisplayLayer.VALUE_LAYER_KEY) != null) {
					obj = ((ValueLayerDisplayLayer) spatial.getProperty(ValueLayerDisplayLayer.VALUE_LAYER_KEY))
							.getProbedObject();
				}
