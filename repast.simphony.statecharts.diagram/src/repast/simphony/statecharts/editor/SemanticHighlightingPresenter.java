...
public class SemanticHighlightingPresenter implements ITextPresentationListener,
  ...
  private class HighlightingPositionUpdater implements IPositionUpdater {
    ...
    /**
     * Update the given position with the given event. The event precedes the
     * position.
     * 
     * @param position
     *          The position
    ...
    /**
     * Update the given position with the given event. The event succeeds the
     * position.
     * 
     * @param position
     *          The position
    ...
    /**
     * Update the given position with the given event. The event is included by
     * the position.
     * 
     * @param position
     *          The position
    ...
    /**
     * Update the given position with the given event. The event overlaps with
    ...
    /**
     * Update the given position with the given event. The event includes the
     * position.
     * 
     * @param position
     *          The position
  ...
  private GroovySourceViewer fSourceViewer;
  ...
  public TextPresentation createPresentation(List addedPositions, List removedPositions) {
    GroovySourceViewer sourceViewer = fSourceViewer;
   ...
   * positions and the existing deleted positions. Also unregisters the deleted
   * positions from the document and patches the positions of this presenter.
   * <p>
   * NOTE: Indirectly called from background thread by UI runnable.
   * </p>
   * 
   * @param textPresentation
   *          the text presentation or <code>null</code>, if the presentation
  ...
  public void updatePresentation(TextPresentation textPresentation,
    ...
    try {
      synchronized (fPositionLock) {
         ...
         * List<Position>, each sorted by position.offset, into one new list.
         * The first of the two is the previous list of positions
         * (oldPositions), from which any deleted positions get removed on the
         * fly. The second of two is the list of added positions. The result is
         * stored in newPositions.
         */
   ...
   * @param offset
   *          the offset
   * @return the index of the last position with an offset equal or greater than
   *         the given offset
   */
   ...
   *          the background presentation reconciler, can be <code>null</code>,
   *          in that case
   *          {@link SemanticHighlightingPresenter#createPresentation(List, List)}
   *          should not be called
   */
  public void install(GroovySourceViewer sourceViewer,
  ...
  /**
   * Add a position with the given range and highlighting unconditionally, only
   * from UI thread. The position will also be registered on the document. The
   * text presentation is not invalidated.
   * 
   * @param offset
   *          The range offset
   * @param length
