...
/*
 * https://github.com/groovy/groovy-eclipse/blob/master/ide/org.codehaus.groovy.eclipse.ui/src/org/codehaus/groovy/eclipse/editor/highlighting/GroovySemanticReconciler.java
 * commit 99d0350e6fd23bb1b55b80e61841582ae557c99c
 */
...
public class GroovySemanticReconciler implements IJavaReconcilingListener {
    ...
    /*
     * private static Method GET_HIGHLIGHTING = null; private static Constructor<?>
     * HIGHLIGHTING_STYLE; private static Constructor<?> HIGHLIGHTED_POSITION;
     * static { try { Class<?> style = Class.forName(
     * "org.eclipse.jdt.internal.ui.javaeditor.SemanticHighlightingManager$Highlighting"
     * ); HIGHLIGHTING_STYLE = ReflectionUtils.getConstructor(style,
     * TextAttribute.class, boolean.class);
     * 
     * Class<?> position = Class.forName(
     * "org.eclipse.jdt.internal.ui.javaeditor.SemanticHighlightingManager$HighlightedPosition"
     * ); HIGHLIGHTED_POSITION = ReflectionUtils.getConstructor(position, int.class,
     * int.class, style, Object.class);
     * 
     * GET_HIGHLIGHTING = position.getDeclaredMethod("getHighlighting");
     * GET_HIGHLIGHTING.setAccessible(true);
     * 
     * } catch (ClassNotFoundException cnfe) { HIGHLIGHTING_STYLE = null;
     * HIGHLIGHTED_POSITION = null;
     * GroovyPlugin.getDefault().logError("Semantic highlighting disabled", cnfe); }
     * catch (NoSuchMethodException nsme) { } }
     */

    private static final String GROOVY_HIGHLIGHT_PREFERENCE = PreferenceConstants.GROOVY_EDITOR_HIGHLIGHT_GJDK_COLOR
	    .replaceFirst("\\.color$", "");
    private static final String STRING_HIGHLIGHT_PREFERENCE = PreferenceConstants.GROOVY_EDITOR_HIGHLIGHT_STRINGS_COLOR
	    .replaceFirst("\\.color$", "");
    ...
    private Highlighting mapKeyHighlighting;
    private Highlighting tagKeyHighlighting;
    private Highlighting numberRefHighlighting;
    private Highlighting regexpRefHighlighting;
    private Highlighting undefinedRefHighlighting;
    private Highlighting deprecatedRefHighlighting;
    private Highlighting staticFieldHighlighting;
    private Highlighting staticValueHighlighting;

    private Highlighting localHighlighting;
    private Highlighting paramHighlighting;

    private Highlighting objectFieldHighlighting;
    private Highlighting methodDefHighlighting;
    private Highlighting methodUseHighlighting;
    private Highlighting groovyMethodUseHighlighting;
    private Highlighting staticMethodUseHighlighting;
	...
	regexpRefHighlighting = newHighlightingStyle(stringColor,
		SWT.ITALIC | loadStyleFrom(prefs, STRING_HIGHLIGHT_PREFERENCE));
	...
	objectFieldHighlighting = newHighlightingStyle(objectFieldColor,
		loadStyleFrom(prefs, OBJECT_FIELD_HIGHLIGHT_PREFERENCE));
	staticFieldHighlighting = newHighlightingStyle(staticFieldColor,
		loadStyleFrom(prefs, STATIC_FIELD_HIGHLIGHT_PREFERENCE));
	staticValueHighlighting = newHighlightingStyle(staticValueColor,
		loadStyleFrom(prefs, STATIC_VALUE_HIGHLIGHT_PREFERENCE));

	methodDefHighlighting = newHighlightingStyle(methodDeclColor,
		loadStyleFrom(prefs, METHOD_DECLARATION_HIGHLIGHT_PREFERENCE));
	methodUseHighlighting = newHighlightingStyle(methodCallColor,
		loadStyleFrom(prefs, OBJECT_METHOD_HIGHLIGHT_PREFERENCE));
	groovyMethodUseHighlighting = newHighlightingStyle(groovyColor,
		loadStyleFrom(prefs, GROOVY_HIGHLIGHT_PREFERENCE));
	staticMethodUseHighlighting = newHighlightingStyle(staticCallColor,
		loadStyleFrom(prefs, STATIC_METHOD_HIGHLIGHT_PREFERENCE));
    }

    protected Highlighting newHighlightingStyle(Color color) {
	return new Highlighting(new TextAttribute(color), true);
	// return ReflectionUtils.invokeConstructor(HIGHLIGHTING_STYLE, new
	// TextAttribute(color), Boolean.TRUE);
    }

    protected Highlighting newHighlightingStyle(Color color, int style) {
	return new Highlighting(new TextAttribute(color, null, style), true);
	// return ReflectionUtils.invokeConstructor(HIGHLIGHTING_STYLE, new
	// TextAttribute(color, null, style), Boolean.TRUE);
    }
	...
	if (!prefs.contains(which + ".enabled") || prefs.getBoolean(which + ".enabled")) {
	    ...
	    // color =
	    // PreferenceConverter.getColor(GroovyPlugin.getDefault().getPreferenceStore(),
    ...
    public void install(GroovyEditor editor, GroovySourceViewer viewer) {
	this.editor = editor;
	this.presenter = new SemanticHighlightingPresenter();
	presenter.install(viewer,
		(JavaPresentationReconciler) editor.getGroovyConfiguration().getPresentationReconciler(viewer));
    }
	...
	return (List<Position>) ReflectionUtils.getPrivateField(SemanticHighlightingPresenter.class, "fPositions",
		presenter);
    }
	...
	if (ast != null && synchronize())
		...
		if (unit != null) {
		    ...
		    Collection<HighlightedTypedPosition> semanticReferences = finder
			    .findSemanticHighlightingReferences();
		    if (update(monitor, 5))
			return;
			...
			if (ref.compareTo(last) != 0) {
			    HighlightedPosition pos = newHighlightedPosition(ref);
			    x = tryAddPosition(newPositions, oldPositions, pos);

			} else if (ref.kind == DEPRECATED || ref.kind == UNKNOWN) {
			    // this and last cover same source range and this indicates deprecated or
			    // unknown
				...
				if (ref.compareTo(x) == 0) {
				    ...
				    GroovyPlugin.getDefault()
					    .logWarning(String.format("Failed to apply %s semantic at %s",
						    ref.kind.name().toLowerCase(), ((Position) ref).toString()));
				    continue; // logic error?
				}
			    }
			    Highlighting style = ((HighlightedPosition) pos).getHighlighting(); // GET_HIGHLIGHTING.invoke(pos);
			    TextAttribute one = getTextAttribute(style);
			    TextAttribute two = getTextAttribute(
				    ref.kind == DEPRECATED ? deprecatedRefHighlighting : undefinedRefHighlighting);
			    // merge the text styling assigned to deprecated or unknown (usually
			    // strikethrough for deprecated and underline for unknown)
			    ((HighlightedPosition) pos).setStyle(
				    newHighlightingStyle(one.getForeground(), one.getStyle() | two.getStyle()));
			    // ReflectionUtils.setPrivateField(pos.getClass(), "fStyle", pos,
			    // newHighlightingStyle(one.getForeground(), one.getStyle() | two.getStyle()));
			}
			last = ref;
		    }
	...
	// return (Position) ReflectionUtils.invokeConstructor(HIGHLIGHTED_POSITION,
	// pos.offset, pos.length, style, this);
    }

    private Position tryAddPosition(List<Position> newPositions, List<Position> oldPositions,
	    HighlightedPosition maybePosition) {
	// TODO: Is there a quicker way to search for matches? These can be sorted
	// easily.
	for (Iterator<Position> it = oldPositions.iterator(); it.hasNext();) {
	    Position oldPosition = it.next();
	    if (!oldPosition.isDeleted() && oldPosition.equals(maybePosition)
		    && isSameStyle(oldPosition, maybePosition)) {
	...
	return ((HighlightedPosition) a).getHighlighting() == ((HighlightedPosition) b).getHighlighting();
    }
