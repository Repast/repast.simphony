...
public class ParametersUIParser {
	...
	private static final QName HIDE_A = new QName("hide");
	private static final QName ORDER_A = new QName("displayOrder");
	...
	public void read(ParametersUI paramsUI, File file) throws FileNotFoundException, XMLStreamException {
	...
	private void processParameter(StartElement elmt, ParametersUI creator) {
		...
		String range = elmt.getAttributeByName(RANGE_A) == null ? null : elmt.getAttributeByName(RANGE_A).getValue();
		// don't need the values because those should be in the parameter's
		// schema
		boolean isList = elmt.getAttributeByName(VALUES_A) != null;
		boolean readOnly = elmt.getAttributeByName(IS_READ_ONLY_A) == null ? false
				: Boolean.parseBoolean(elmt.getAttributeByName(IS_READ_ONLY_A).getValue());

		boolean hide = elmt.getAttributeByName(HIDE_A) == null ? false
				: Boolean.parseBoolean(elmt.getAttributeByName(HIDE_A).getValue());
		

		double displayOrder = ParametersUI.LAST;
		try {
			displayOrder = elmt.getAttributeByName(ORDER_A) == null ? ParametersUI.LAST : Double.parseDouble(elmt.getAttributeByName(ORDER_A).getValue());
		} catch (NumberFormatException ex) {
			throw new IllegalArgumentException("Parameter display order attribute must be a number");
		}

		if (!hide) {
			if (readOnly) {
				creator.addBinder(group, new ReadOnlyParameterBinder(name, displayName), displayOrder);
				...
				if (!(type.equals("int") || type.equals("double") || type.equals("float")))
					throw new IllegalArgumentException("Ranged parameters must be of the int, double, or float type");
				...
				if (type.equals("int")) {
					try {
						...
						creator.addBinder(group, new RangeParameterBinder(name, displayName, min, max, spacing), displayOrder);
					} catch (NumberFormatException ex) {
						throw new IllegalArgumentException("Invalid range for ranged parameter '" + name + "'");
					}
				} else {
					...
					try {
						...
						creator.addBinder(group, new FPRangeParameterBinder(name, displayName, min, max, spacing), displayOrder);
					} catch (NumberFormatException ex) {
						throw new IllegalArgumentException("Invalid range for ranged parameter '" + name + "'");
					}
				}
			} else if (isList) {
				creator.addBinder(group, new ListParameterBinder(name, displayName), displayOrder);
			} else if (type.equals("boolean")) {
				creator.addBinder(group, new BooleanParameterBinder(name, displayName), displayOrder);
			} else {
				Class<?> clazz = typeMap.get(type);
				if (clazz == null)
					clazz = Object.class;
				creator.addBinder(group, new DefaultParameterBinder(name, displayName, clazz), displayOrder);
			}
		}
	}
}
