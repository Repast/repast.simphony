...
public class ProbeModel extends PresentationModel<Object> {
  ...
  private static class ProbeBeanAdapter extends BeanAdapter<Object> {
    ...
    protected SimplePropertyAdapter createPropertyAdapter(String propertyName, String getterName,
        String setterName) {
      SimplePropertyAdapter adapter = new SimplePropertyAdapter(propertyName, getterName,
          setterName);
    ...
    public class SimplePropertyAdapter extends BeanAdapter<Object>.SimplePropertyAdapter {
      ...
      protected void fireChange(final Object oldValue, final Object newValue) {
   ...
   * 
   * @see
   * com.jgoodies.binding.PresentationModel#createBeanAdapter(com.jgoodies.binding
   * .value.ValueModel)
   */
  // This is necessary to override the default PresentationModel behavior that
  // requires the
  // bean for which this is a model to implement addPropertyChangeListener,
  // removePropertyChangeListener.
  // We are working with arbitrary objects that most likely don't conform to the
  // bean spec.
  @Override
  ...
  /**
   * Updates any bindings associated with this model to reflect the current
   * state of the probed object.
   * 
   */
  public void update() {
    adapter.fireChange();
