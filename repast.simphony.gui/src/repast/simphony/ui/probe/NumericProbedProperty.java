...
public class NumericProbedProperty extends ProbedProperty {
	...
	public NumericProbedProperty(PropertyDescriptor desc, List possibleValues,
			boolean wrap) {
		this(desc, wrap);
	...
	public JComponent getComponent(PresentationModel model, boolean buffered) {
		AbstractValueModel valueModel = buffered ? model.getBufferedModel(name)
				: model.getModel(name);
		if (type == Type.READ) {
			return this.wrapWithSparkLineButton(model, BasicComponentFactory
					.createLabel(valueModel, Utils.getNumberFormatInstance()));
		}

		if (values != null) {
			return this.wrapWithSparkLineButton(model, new JComboBox(
					new ComboBoxAdapter(values, valueModel)));
		}

		switch (numType) {
		case BIGINTEGER: {
			EmptyNumberFormatter formatter = new ZeroNumberFormatter(Utils
					.getNumberFormatInstance());
			formatter.setValueClass(BigInteger.class);
			return this.wrapWithSparkLineButton(model, BasicComponentFactory
					.createFormattedTextField(valueModel, formatter));
		}
		case BIGDECIMAL: {
			EmptyNumberFormatter formatter = new ZeroNumberFormatter(Utils
					.getNumberFormatInstance());
		...
		default:
			return this.wrapWithSparkLineButton(model, BasicComponentFactory
					.createIntegerField(valueModel, 0));
		}
	}

	public JComponent wrapWithSparkLineButton(PresentationModel model,
		...
		if (this.wrap) {
			...
			SparklineJComponent sparklineButton = new PropertySourcedSparklineJComponent(
					model, this);
...
class ZeroNumberFormatter extends EmptyNumberFormatter {

	public ZeroNumberFormatter() {
		super();
	}

	public ZeroNumberFormatter(NumberFormat numberFormat) {
		super(numberFormat, 0);
	}
