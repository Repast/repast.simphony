...
public class RepastProjectClasspathContainer extends
		ClasspathContainerInitializer implements IClasspathContainer {
	...
	private static final String DESCRIPTION = AgentBuilderPlugin
			.getResourceString("Classpath_Container.description");
		...
		if (containerPath.equals(PATH)) {
			JavaCore
					.setClasspathContainer(
							...
							new IClasspathContainer[] { new RepastProjectClasspathContainer() },
							null);
		}
	}
	...
	public IClasspathEntry[] getClasspathEntries() {
		...
		if (classpathList == null) {
			...
			String[] jarPathList = AgentBuilderPlugin.getJarPathList();
			...
			for (int i = 0; i < jarPathList.length; i++) {
				...
				classpathList[i] = JavaCore
						.newLibraryEntry(jarPath, null, null);
			}
