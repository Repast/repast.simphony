...
public class ZeroMQTest2 {
  ...
  private static class server_task implements Runnable {
    ...
    public void run() {
      ...
      while (!Thread.currentThread().isInterrupted()) {
        ...
        if (poller.pollin(0)) {
          ...
          for (int reply = 0; reply < replies; reply++) {
            ...
            content.send(frontend, ZFrame.REUSE);
          }
          address.destroy();
          content.destroy();
        }
      ...
      /*
       * // Backend socket talks to workers over inproc Socket backend =
       * ctx.createSocket(ZMQ.DEALER); backend.bind("inproc://backend");
       * 
       * // Launch pool of worker threads, precise number is not critical for
       * (int threadNbr = 0; threadNbr < 5; threadNbr++) new Thread(new
       * server_worker(ctx)).start();
       * 
       * // Connect backend to frontend via a proxy ZMQ.proxy(frontend, backend,
       * null);
       */
