...
import repast.simphony.agents.designer.ui.wizards.NewCodeWizard;
...
public class RepastCodeWizardAction extends Action implements
	...
	public void run(IAction action) {
		...
		EditableComboBoxCellEditor cellEditor = AgentEditor
		...
		if (cellEditor == null) {
			...
			if (part instanceof AbstractTextEditor) {
				...
				if (results != null) {
					...
					try {
						ISelection selection = page.getActiveEditor()
								.getEditorSite().getSelectionProvider()
								.getSelection();
						if (selection instanceof ITextSelection) {
							int start = ((ITextSelection) selection)
									.getOffset();
...
//		action.setEnabled(this.isEnabled());
	...
	public static void checkDefaultList() {
		...
		if (defaultEntryList == null) {
			...
			ArrayList<NewCodeWizardEntry> taskList = TaskWizardGenerator
					.createTaskWizard();
			...
			ArrayList<NewCodeWizardEntry> decisionList = DecisionWizardGenerator
			...
			ArrayList<NewCodeWizardEntry> loopList = LoopWizardGenerator
					.createLoopWizard();
			...
			defaultEntryList.add(new NewCodeWizardRadioButtonEntry(
					"Insert Repast Code",
					"Please Select the Kind of Code to Insert", new String[] {
							"Task Step", "Decision Step", "Loop Step" }, null));
	...
	public String runWizardDialog(ArrayList<NewCodeWizardEntry> entryList) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Error Creating the Wizard Titled \""
					+ entryList.get(0).title + "\"");
		}
