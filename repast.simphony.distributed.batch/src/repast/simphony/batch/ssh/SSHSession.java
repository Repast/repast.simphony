...
public class SSHSession {
	...
	/*
	 * Lists the non-directory contents the specified remote directory and places
	 * the results into the files parameters. If recurse is true, then this will
	 * recurse into any directories and list the contents of those directories.
	 * 
	 * @param sftp
	 * 
	 * @param remoteDir
	...
	private void listRemoteDirectory(ChannelSftp sftp, String remoteDir, List<String> files, boolean recurse)
			throws SftpException {
...
//				if (attrs.isLink()) {
//					String target = sftp.readlink(remoteDir + "/" + filename);
//					SftpATTRS linkAttrs = sftp.stat(target);
//					if (linkAttrs.isDir()) {
//						subs.add(target);
//					}
//				} else {
				if (recurse && attrs.isDir()) {
	...
	/**
	 * Returns the file listing of the specified remote directory as a list of
	 * Strings with an option to recurse into any directories that are found. If
	 * recurse is true then the files in the subdirectories will be returned with
	 * that subdirectory prefixed to the file name (e.g. output/file1).
	 * 
	 * @param remoteDir
	 * @return
	 * @throws JSchException
	...
	/**
	 * Execute the specified command on the remote machine and append any output to
	 * the StringBuilder. If setErrStream is true then errors will be directory to a
	 * LoggingOutputStream.
	 * 
	 * @param cmd
	 * @param builder
	...
	public int executeCmd(String cmd, StringBuilder builder, boolean setErrStream) throws JSchException, IOException {
		...
		try {
			...
			if (setErrStream)
				((ChannelExec) channel).setErrStream(new PrintStream(new LoggingOutputStream(logger, Level.ERROR)));
	...
	/**
	 * Compies the specified file to the home directory of the user used to log in
	 * to the remote.
	 * 
	 * @param localFile
	 * @throws JSchException
	...
	/**
	 * Copies the specified remote file to the specified local directory. If
	 * preserveRemotePath is true then path will be preserved when the file is
	 * copied. For example, assuming a local directory of "foo", then copying
	 * "output/out.txt" will create "foo/output/out.txt".
	 * 
	 * @param localDir
	 * @param remoteFile
	 ...
	 * @throws JSchException
	 */
	public File copyFileFromRemote(String localDir, File remoteFile, boolean preserveRemotePath)
	 ...
	 * structure. If preserveRemotePath is true then path will be preserved when the
	 * file is copied. For example, assuming a local directory of "foo", then
	 * copying "output/out.txt" will create "foo/output/out.txt".
	 * 
	 * 
	 * @param localDir
	 * @param remoteFiles
