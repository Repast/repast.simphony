 ...
 */
public class ContextGrid<T> extends DefaultGrid<T> implements
        ContextListener<T> {
   ...
   */
  public ContextGrid(String name, GridAdder<T> adder,
                     GridPointTranslator translator, CellAccessor<T, Map<GridPoint, Object>> accessor,
                     int... size) {
  ...
  /**
   * Constructs this space with the specified name, adder, translator,
   * accessor, size, and origin. The size is the size of the space meaning [3, 3] is a
   * 3x3 space. The origin is the offset on the space where the [0,0] grid point is.
   *
   * @param name   the name of the space
   * @param size   the dimensions of the space
