...
public class ParametersParser extends DefaultHandler2 {
	...
	public ParametersParser(Parameters params, File file)
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		try {
			if (qName.equals(PARAMETER)) {
				...
				if (name.equals(ParameterConstants.DEFAULT_RANDOM_SEED_USAGE_NAME) && defVal == null
						&& creator.contains(ParameterConstants.DEFAULT_RANDOM_SEED_USAGE_NAME))
					return;
				...
				if (range != null) {
                    ...
                    if (vals.length != 3)
                    	 throw new SAXException("Invalid range for ranged parameter '" + name + "'");
                    try {
						SteppedRange sr = new SteppedRange(Double.parseDouble(vals[0]), Double.parseDouble(vals[1]),
								Double.parseDouble(vals[2]));
						creator.addConstraint(name, sr);
                    } catch (NumberFormatException ex) {
                    	throw new SAXException("Invalid range for ranged parameter '" + name + "'. Values must be numeric");
                    }
				}
			}
