...
public class PropertyGreaterThan<T> extends AbstractPropertyQuery<T> {
	...
	private class GreaterThan<T> implements Predicate<T> {
		...
		public boolean evaluate(T t) {
			...
			if (method != null) {
				try {
					...
					AbstractPropertyQuery.msgCenter.error("Error evaluting property greater than predicate", e);
				}
			}
			return false;
		}
	...
	protected Predicate<T> createPredicate(Context<T> context, String propertyName) {
		try {
			...
			return new GreaterThan<T>(methodMap);
