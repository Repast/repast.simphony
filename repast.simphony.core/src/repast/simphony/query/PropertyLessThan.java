...
public class PropertyLessThan<T> extends AbstractPropertyQuery<T> {
	...
	private class LessThan<T> implements Predicate<T> {

		Map<Class, FastMethod> methodMap;

		public LessThan(Map<Class, FastMethod> methodMap) {
		...
		public boolean evaluate(T t) {
			...
			if (method != null) {
				try {
					...
					AbstractPropertyQuery.msgCenter.error("Error evaluting property less than predicate", e);
				}
			}
			return false;
		}
	 ...
	 */
	public PropertyLessThan(Context<T> context, String propertyName, Number propertyValue) {
	 ...
	 * @param propertyName
	 *
	 * @return a Predicate that tests if a numeric property is less than some value.
	 */
	protected Predicate<T> createPredicate(Context<T> context, String propertyName) {
		try {
			...
			return new LessThan<T>(methodMap);
