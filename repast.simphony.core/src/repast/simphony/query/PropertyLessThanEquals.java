...
public class PropertyLessThanEquals<T> extends AbstractPropertyQuery<T> {
	...
	private class LessThanEquals<T> implements Predicate<T> {

		Map<Class, FastMethod> methodMap;

		public LessThanEquals(Map<Class, FastMethod> methodMap) {
		...
		public boolean evaluate(T t) {
			...
			if (method != null) {
				try {
					...
					AbstractPropertyQuery.msgCenter.error("Error evaluting property less than or equals predicate", e);
				}
			}
			return false;
		}
	...
	protected Predicate<T> createPredicate(Context<T> context, String propertyName) {
		try {
			...
			return new LessThanEquals<T>(methodMap);
